...
class SEAMParameterSweep:
    ...
    def load_keras_model(self, model_path: str):
        ...
        # Apply SEAM intgrad patch before loading model
        print("Applying SEAM integrated gradients patch...")
        from simple_seam_intgrad import patch_seam_intgrad
        patch_seam_intgrad()
        ...
        # Test patched integrated gradients
        print("Testing patched integrated gradients...")
        from simple_seam_intgrad import SimpleSeamAnalyzer
        test_analyzer = SimpleSeamAnalyzer(gpu_id=self.gpu_id)
        test_analyzer.keras_model = self.keras_model
        
        # Test with a small sequence in (seq_length, alphabet) format for SEAM
        test_seq = tf.random.normal((249, 4)).numpy()
        
        # Test Dev task
        dev_attr = test_analyzer.compute_intgrad(test_seq, task_index=0, steps=10)
        print(f"✓ Dev task attribution: range {dev_attr.min():.6f} to {dev_attr.max():.6f}")
        
        # Test Hk task  
        hk_attr = test_analyzer.compute_intgrad(test_seq, task_index=1, steps=10)
        print(f"✓ Hk task attribution: range {hk_attr.min():.6f} to {hk_attr.max():.6f}")
        
        # Verify they're different
        correlation = np.corrcoef(dev_attr.flatten(), hk_attr.flatten())[0, 1]
        print(f"✓ Task correlation: {correlation:.6f} (should be < 1.0)")
        print(f"✓ Patched integrated gradients working correctly!")
    ...
    def load_reference_attributions(self, reference_dir: str, task_index: int = 0):
        ...
        ref_files = glob.glob(os.path.join(reference_dir, pattern))
        ...
        def get_seq_number(filename):
            ...
            return int(basename.split('_')[-1].split('.')[0].replace('seq', ''))
        
        for ref_file in sorted(ref_files, key=get_seq_number):
            # Extract sequence index from filename
            filename = os.path.basename(ref_file)
            seq_idx = int(filename.split('_')[-1].split('.')[0].replace('seq', ''))
            
            print(f"  Loading sequence {seq_idx}...")
            try:
    ...
    def generate_mutagenesis_library(self, reference_sequence: np.ndarray, 
        ...
        # SQUID generates (seq_length, alphabet) which is what SEAM expects
        # No transpose needed! SEAM expects (batch, seq_length, alphabet)
        
        # Convert to float32 for SEAM compatibility
        x_mut = x_mut.astype(np.float32)
    ...
    def compute_attributions(self, x_mut: np.ndarray, y_mut: np.ndarray, x_ref: np.ndarray, 
        ...
        # Transpose x_ref from (1, alphabet, seq_length) to (1, seq_length, alphabet) for SEAM
        x_ref_transposed = np.transpose(x_ref, (0, 2, 1))
        
        # SEAM API - Compile sequence analysis data
        compiler = Compiler(
            x=x_mut,
            y=y_mut,
            x_ref=x_ref_transposed,
            y_bg=None,
