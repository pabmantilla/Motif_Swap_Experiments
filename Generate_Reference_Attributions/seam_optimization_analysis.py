...
class SEAMOptimizationAnalysis:
    ...
    def load_keras_model(self, model_path: str):
        ...
        test_seq = tf.random.normal((4, 249)).numpy()
    ...
    def run_full_optimization(self, sequences: np.ndarray, task_indices: List[int] = [0, 1], 
        ...
        for task_idx in task_indices:
            ...
            for seq_idx, reference_sequence in enumerate(sequences):
                ...
                # Check if this sequence's reference attribution already exists
                temp_save_path = f"temp_reference_attributions_task{task_idx}_seq{seq_idx}.pkl"
                if os.path.exists(temp_save_path):
                    print(f"  ✓ Found existing reference attribution for sequence {seq_idx}")
                    try:
                        with open(temp_save_path, 'rb') as f:
                            reference_attributions[seq_idx] = pickle.load(f)
                        print(f"  ✓ Loaded existing reference attribution for sequence {seq_idx}")
                        continue
                    except Exception as e:
