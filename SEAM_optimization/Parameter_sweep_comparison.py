...
def run_comparison_for_sequence(seq_index, py_dir):
    ...
    for parameter_sweep_index in range(len(parameter_sweep_set)):
        ...
        # Handle both old and new format for reference data
        ref_bg = ref_data.get('meta_background', ref_data.get('background'))
        ref_fg = ref_data.get('wt_cluster_foreground', ref_data.get('ref_cluster_avg'))
        ref_bg_cluster = ref_data.get('wt_cluster_specific_background', ref_data.get('background'))
        
        # Handle both old and new format for parameter data
        param_bg = param_data.get('meta_background', param_data.get('background'))
        param_fg = param_data.get('wt_cluster_foreground', param_data.get('ref_cluster_avg'))
        param_bg_cluster = param_data.get('wt_cluster_specific_background', param_data.get('background'))
...
def load_arrow_data(filepath):
    ...
    if len(shapes) >= 6:  # New format with reference attribution
        ...
        # Use wt_cluster_foreground as ref_cluster_avg for backward compatibility
        ref_cluster_avg = wt_cluster_foreground
        ...
        wt_cluster_foreground = np.frombuffer(df['wt_cluster_foreground'][0], dtype=wt_fg_dtype).reshape(wt_fg_shape)
        wt_cluster_background = np.frombuffer(df['wt_cluster_specific_background'][0], dtype=wt_bg_dtype).reshape(wt_bg_shape)
        ref_cluster_avg = np.frombuffer(df['wt_cluster_foreground'][0], dtype=ref_cluster_dtype).reshape(ref_cluster_shape)  # Use wt_cluster_foreground as ref_cluster_avg
        background_sequence = np.frombuffer(df['background_sequence_onehot'][0], dtype=bg_seq_dtype).reshape(bg_seq_shape)
        reference_attribution = None
