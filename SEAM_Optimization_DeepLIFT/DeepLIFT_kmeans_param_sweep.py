...
# CRITICAL: Set GPU before any CUDA/PyTorch imports
...
# Resolve CSV relative to the script location for batch robustness
script_dir = os.path.dirname(os.path.abspath(__file__))
test_library_path = os.path.abspath(os.path.join(script_dir, '..', '..', '..', 'test_library_df.csv'))
...
PERSISTENT_NUM_SEQS_SWEEP = [100, 500, 1000, 5000, 10000, 50000, 75000, 100000]  # number of sequences to generate (5K for debugging)
PERSISTENT_N_CLUSTERS_SWEEP = [10, 20, 30, 40, 50, 75]  # number of clusters for  clustering
PERSISTENT_CLUSTERING_METHOD_SWEEP = ['kmeans']  # clustering method (hierarchical already done by hierarchal_sweep.py)
...
def run_parameter_sweep(parameter_sweep_index, seq_index, task_index):
    ...
    seq_folder = os.path.join(
        f"parameter_set_{num_seqs}_{n_clusters}_{clustering_method}_results",
        f"Seq{seq_index}"
    )
    ...
    # Aggressive memory cleanup before K-means
    ...
    # =============================================================================
    tf.keras.backend.clear_session()  # Clear any existing TF session
    ...
    if clustering_method == 'kmeans':  # Direct K-Means clustering on original feature space
        ...
        # Check GPU memory before K-means
        if torch.cuda.is_available():
            ...
            print(f"GPU memory free before K-means: {gpu_memory_free_gb:.2f}GB")
            
            # Force CPU if less than 8GB free (kmeanstf needs ~1.5GB + buffer)
            if gpu_memory_free_gb < 8.0:
                print("WARNING: Insufficient GPU memory for K-means, forcing CPU mode")
                gpu_for_kmeans = False
            else:
                gpu_for_kmeans = gpu
        else:
            gpu_for_kmeans = False
        ...
        clusterer = Clusterer(
            ...
            gpu=gpu_for_kmeans  # Use GPU only if sufficient memory
        )
        ...
        dummy_centroids = np.zeros((n_clusters, attributions_pool.shape[1] * attributions_pool.shape[2]))
    ...
    fig, axes = plt.subplots(7, 1, figsize=(12, 21))
    fig.suptitle(f'Seq{seq_index} - {num_seqs} sequences, {n_clusters} clusters, {clustering_method} clustering', fontsize=12, y=0.99)
    ...
    combined_logo_filename = f'Seq{seq_index}_({num_seqs} sequences, {n_clusters} clusters, {clustering_method} clustering).png'
    ...
    table = pa.table({
        ...
        'wt_cluster_average': [WT_cluster_avg_raw.tobytes()],
        
        
        # Array shapes and dtypes for reconstruction
        'array_shapes': [str(average_background_attribution.shape) + '|' + str(WT_cluster_specific_foreground.shape) + '|' + str(WT_cluster_scaled_foreground.shape) + '|' + str(cluster_background.shape) + '|' + str(scaled_background.shape) + '|' + str(WT_cluster_avg_raw.shape)],
        'array_dtypes': [str(average_background_attribution.dtype) + '|' + str(WT_cluster_specific_foreground.dtype) + '|' + str(WT_cluster_scaled_foreground.dtype) + '|' + str(cluster_background.dtype) + '|' + str(scaled_background.dtype) + '|' + str(WT_cluster_avg_raw.dtype)],
    ...
    metadata = {
        ...
        b'description': b'SEAM attribution maps: average_background_attribution, WT_cluster_specific_foreground, WT_cluster_scaled_foreground, cluster_background, scaled_background, WT_cluster_average',
    ...
    save_path_essential = os.path.join(
        f"parameter_set_{num_seqs}_{n_clusters}_{clustering_method}_results",
        f"Seq{seq_index}"
    )
...
for parameter_sweep_index in range(len(parameter_sweep_set)):
    parameter_sweep_current = parameter_sweep_set.iloc[parameter_sweep_index]
    for seq_index in range(len(test_library_df)):
        clustering_method = parameter_sweep_current['clustering_method']
        num_seqs = parameter_sweep_current['num_seqs']
        n_clusters = parameter_sweep_current['n_clusters']
        seq_folder = os.path.join(
            f"parameter_set_{num_seqs}_{n_clusters}_{clustering_method}_results",
            f"Seq{seq_index}"
        )
        ...
        run_parameter_sweep(parameter_sweep_index, seq_index, task_index)
